(() => {
    ...
    els.gantt.innerHTML = `<div class="gantt-label-head">Tarefa / prioridade</div>
      ...
      <div class="gantt-labels">${tasks
        .map(
          (
            task,
          ) => `<button type="button" class="gantt-label-row" draggable="true" data-task-label="${task.id}" title="Arraste para trocar a sequência">
  ...
  function fillTaskOptions() {
    ...
    els.taskIcon.innerHTML = Object.entries(state.iconCatalog.icons || {})
      .sort((a, b) =>
        String(a[1].label).localeCompare(String(b[1].label), "pt-BR"),
      )
      .map(
        ([key, item]) =>
          `<option value="${escapeAttr(key)}">${escapeHtml(item.label || key)}</option>`,
      )
      .join("");
  }
  ...
  function openTaskDialog(task = null) {
    ...
    els.taskSequence.value =
      task?.sequence ||
      Math.max(0, ...state.tasks.map((item) => item.sequence)) + 1;
  ...
  async function submitTask(event) {
    ...
    try {
      ...
      setStatus(
        els.appStatus,
        "ok",
        "Tarefa salva. Clique em “Salvar tudo” para enviar a cópia segura.",
      );
    } catch (error) {
