...
class ImportSodimacTests(unittest.TestCase):
    ...
    def _winner(
        self,
        url: str = "https://www.sodimac.com.br/sodimac-br/product/123/cimento-50kg/123/",
    ) -> dict:
        return {
            "matched_product": "Cimento CPII 50 kg",
            "sku": "123",
            "seller": "Sodimac Brasil",
    ...
    def test_merge_import_preserves_financial_state_and_records_cep(self) -> None:
        ...
        self.assertTrue(rows[0]["quotation_id"].startswith("SODIMAC_"))
        self.assertEqual(rows[0]["vendor"], "Sodimac Brasil")
        ...
        self.assertEqual(after["quotations"][0]["vendor"], "Sodimac Brasil")

    def test_rejects_non_product_url(self) -> None:
        self._write_cluster(
            self._winner("https://www.sodimac.com.br/sodimac-br/category/cimento/")
        )
        with patch.object(importer, "SOURCE_DIR", self.source), patch.object(
            importer, "SCOPE_PATH", self.scope
        ):
            with self.assertRaisesRegex(ValueError, "direct HTTPS Sodimac"):
