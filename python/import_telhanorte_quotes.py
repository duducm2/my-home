"""Merge, validate, and optionally import the Telhanorte price sweep."""
...
SOURCE_DIR = RESEARCH_DIR / "telhanorte"
SCOPE_PATH = SOURCE_DIR / "scope.json"
CATALOG_PATH = RESEARCH_DIR / "telhanorte-lowest-prices.json"
CATALOG_MARKDOWN_PATH = RESEARCH_DIR / "telhanorte-lowest-prices.md"
...
ALLOWED_HOSTS = {"telhanorte.com.br", "www.telhanorte.com.br"}
...
def _validate_url(value: Any, expense_id: str) -> str:
    ...
    if (
        ...
        or not path.endswith("/p")
    ):
        raise ValueError(
            f"{expense_id}: product_url must be a direct HTTPS Telhanorte /p page"
        )
...
def merge_catalog(store: ExpenseStore) -> dict[str, Any]:
    ...
    if not cluster_files:
        raise ValueError("no Telhanorte cluster files were found")
    ...
    return {
        "source": "Telhanorte",
        "source_url": "https://www.telhanorte.com.br/",
        "location_basis": LOCATION_BASIS,
        ...
        rows.append(
            {
                ...
                "quotation_id": f"TELHANORTE_{run_date}",
                ...
                "quote_valid_until": "",
                "source_type": "text",
                "source_name": "telhanorte-lowest-prices.json",
                "source_page": "",
...
def _pack_text(rows: list[dict[str, Any]]) -> str:
    ...
    return (
        "===PREVIEW===\n"
        "Lowest verified Telhanorte offers for CEP 13380-000 (Nova Odessa/SP).\n"
...
def _markdown(catalog: dict[str, Any]) -> str:
    ...
    lines = [
        "# Telhanorte — menores preços verificados",
        "",
...
def main() -> int:
    ...
    if not preview["ok"]:
        ...
        return 1
    print(
        f"Validated {len(rows)} Telhanorte winners across "
    ...
    if not result["ok"]:
        ...
        return 1
    print(
        f"Imported {len(rows)} unselected Telhanorte quotations; "
