"""Merge, validate, and optionally import the Leroy Merlin price sweep."""
...
SOURCE_DIR = RESEARCH_DIR / "leroymerlin"
SCOPE_PATH = SOURCE_DIR / "scope.json"
CATALOG_PATH = RESEARCH_DIR / "leroymerlin-lowest-prices.json"
CATALOG_MARKDOWN_PATH = RESEARCH_DIR / "leroymerlin-lowest-prices.md"
...
ALLOWED_HOSTS = {"leroymerlin.com.br", "www.leroymerlin.com.br"}
...
def _validate_url(value: Any, expense_id: str) -> str:
    ...
    if (
        ...
        raise ValueError(
            f"{expense_id}: product_url must be a direct HTTPS Leroy Merlin "
            f"/<slug>_<productId> page"
        )
...
def merge_catalog(store: ExpenseStore) -> dict[str, Any]:
    ...
    if not cluster_files:
        raise ValueError("no Leroy Merlin cluster files were found")
    ...
    return {
        "source": "Leroy Merlin",
        "source_url": "https://www.leroymerlin.com.br/",
        "location_basis": LOCATION_BASIS,
        ...
        rows.append(
            {
                ...
                "quotation_id": f"LEROY_{run_date}",
                ...
                "quote_valid_until": "",
                "source_type": "text",
                "source_name": "leroymerlin-lowest-prices.json",
                "source_page": "",
...
def _pack_text(rows: list[dict[str, Any]]) -> str:
    ...
    return (
        "===PREVIEW===\n"
        "Lowest verified Leroy Merlin offers for CEP 13380-000 (Nova Odessa/SP).\n"
...
def _markdown(catalog: dict[str, Any]) -> str:
    ...
    lines = [
        "# Leroy Merlin — menores preços verificados",
        "",
...
def main() -> int:
    ...
    if not preview["ok"]:
        ...
        return 1
    print(
        f"Validated {len(rows)} Leroy Merlin winners across "
    ...
    if not result["ok"]:
        ...
        return 1
    print(
        f"Imported {len(rows)} unselected Leroy Merlin quotations; "
