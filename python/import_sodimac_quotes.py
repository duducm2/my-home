"""Merge, validate, and optionally import the Sodimac Brasil price sweep."""
...
SOURCE_DIR = RESEARCH_DIR / "sodimac"
SCOPE_PATH = SOURCE_DIR / "scope.json"
CATALOG_PATH = RESEARCH_DIR / "sodimac-lowest-prices.json"
CATALOG_MARKDOWN_PATH = RESEARCH_DIR / "sodimac-lowest-prices.md"
...
ALLOWED_HOSTS = {"sodimac.com.br", "www.sodimac.com.br"}
LOCATION_BASIS = "nova_odessa_cep_13380"
PRODUCT_PATH_RE = re.compile(
    r"^/sodimac-br/product/([^/]+)/([^/]+)/\1/?$",
    re.IGNORECASE,
)
...
def _validate_url(value: Any, expense_id: str) -> str:
    ...
    parsed = urlparse(url)
    path_check = parsed.path.rstrip("/")
    if (
        parsed.scheme != "https"
        or parsed.hostname not in ALLOWED_HOSTS
        or not PRODUCT_PATH_RE.match(path_check)
    ):
        raise ValueError(
            f"{expense_id}: product_url must be a direct HTTPS Sodimac "
            f"/sodimac-br/product/<id>/<slug>/<id>/ page"
        )
...
def merge_catalog(store: ExpenseStore) -> dict[str, Any]:
    ...
    if not cluster_files:
        raise ValueError("no Sodimac cluster files were found")
    ...
    return {
        "source": "Sodimac Brasil",
        "source_url": "https://www.sodimac.com.br/",
        "location_basis": LOCATION_BASIS,
...
def _pack_rows(catalog: dict[str, Any], store: ExpenseStore) -> list[dict[str, Any]]:
    ...
    for item in catalog["expenses"]:
        ...
        caveats.append(
            "Preço referenciado para CEP 13380-000 (Nova Odessa/SP); confirmar frete no checkout."
        )
        ...
        rows.append(
            {
                ...
                "quote_valid_until": "",
                "source_type": "text",
                "source_name": "sodimac-lowest-prices.json",
                "source_page": "",
...
def _pack_text(rows: list[dict[str, Any]]) -> str:
    ...
    return (
        "===PREVIEW===\n"
        "Lowest verified Sodimac Brasil offers for CEP 13380-000 (Nova Odessa/SP).\n"
...
def _markdown(catalog: dict[str, Any]) -> str:
    ...
    lines = [
        "# Sodimac Brasil — menores preços verificados",
        "",
    ...
    for item in catalog["expenses"]:
        ...
        lines.extend(
            [
                ...
                f"- Ressalvas: {'; '.join(winner['caveats']) or 'Somente a ressalva de CEP/frete indicada acima.'}",
                "",
            ]
        )
...
def main() -> int:
    ...
    if not preview["ok"]:
        ...
        return 1
    print(
        f"Validated {len(rows)} Sodimac winners across "
    ...
    if not result["ok"]:
        ...
        return 1
    print(
        f"Imported {len(rows)} unselected Sodimac quotations; "
